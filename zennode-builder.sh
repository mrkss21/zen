...
function CheckUser(){
	if [[ $USER == "root" ]]; then
		echo "Setup and running ZenCash node from root insecure, can't contine."
		echo "Please add new user (here 'zuser' for example):"
		echo "   useradd -m -G sudo,systemd-journal,adm -s /bin/bash zuser"
		echo "Set user password:"
		echo "   passwd zuser"
		echo "And login via ssh or:"
		echo"   su - zuser"
		ExitErrorMessage ""
	fi
}
...
function ProcessError(){
	...
	echo >&2 "Please contact @640kilobyte on official ZenCash Discord  channel or ZenCash channel at Telegram messenger."
	echo "Last 5 lines:"
	tail -n5 $INSTALLLOG
}
